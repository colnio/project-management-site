/**
 * Modal to add, edit, and remove project experiment tags.
 */
import { useState } from 'react';
import {
  useProjectExperimentTags,
  useCreateExperimentTag,
  useUpdateExperimentTag,
  useDeleteExperimentTag,
  type ExperimentTag,
...
function TagRow({
  ...
  const updateTag = useUpdateExperimentTag(projectId, tag.id);
  const deleteTag = useDeleteExperimentTag(projectId);
...
export function ExperimentTagsManager({ projectId, onClose }: ExperimentTagsManagerProps) {
  const { data: tags = [], isLoading, isError } = useProjectExperimentTags(projectId);
  const createTag = useCreateExperimentTag(projectId);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
        <div className="modal-head">
          <span className="modal-title">Experiment tags</span>
          <button type="button" className="icon-btn" onClick={onClose}>✕</button>
        </div>
        <div className="modal-body">
          <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--muted)', lineHeight: 1.55 }}>
            Define tags for this project. Assign one or more from each experiment&apos;s page or when creating an experiment.
          </p>
