...
The workflow engine (G3/G4) and AI frontend (G6/G7/G8) are not yet built.
...
unified; FullCalendar's premium timeline is deliberately avoided). OpenAPI
TypeScript types are generated by `pnpm openapi`. MSW backs the Vitest suite.

## Not yet built

- **Track G3/G4** risk-workflow engine + library, and **G6/G7/G8** the AI
  frontend (chat panel, workflow runner, autonomy config UI). The G1/G2/G5 chat
  backend above is done. All of Tracks A–F are complete.
