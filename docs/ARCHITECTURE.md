...
follow-up). Endpoints: `GET /v1/ai/workflows`, `POST /v1/ai/workflows/{key}/run`,
`GET /v1/ai/runs/{id}`.

The AI frontend (G6/G7/G8) is not yet built.
...
- **Track G6/G7/G8** — the AI frontend (chat panel, workflow runner, autonomy
  config UI). The entire AI backend (G1–G5) and all of Tracks A–F are complete.
